...
__WEAK void app_error_handler(uint32_t error_code, uint32_t line_num, const uint8_t * p_file_name)
{
	...
	SSD1306_NumWDesc(0,0,"Code:",error_code);
	SSD1306_NumWDesc(0,8,"Line:",line_num);
	
	int len = strlen((char*)p_file_name);
	SSD1306_NumWDesc(0,16,"fuck:",len);
	...
	for(i=0; i<len/10; i++)
	{
		for(uint8_t j=0; j<10; j++)
			temp[j] = p_file_name[i*10+j];
		
		SSD1306_String(0,127-8*i, temp);
	}
	
	for(uint8_t j=0; j<10; j++)	temp[j]='\0';
	
	if(len%10)
	{
		for(uint8_t j=0; j<len%10; j++)
			temp[j] = p_file_name[i*10+j];
