...
#include "app_util.h"
#include "../../../RTE/Application/dispatcher.h"
...
static dispID				  m_conn_params_timer_id;
...
{
	DispatcherRemoveEvnt(m_conn_params_timer_id);
	return NRF_SUCCESS;
}
...
static void conn_params_negotiation(void)
{
    ...
    if (!is_conn_params_ok(&m_current_conn_params))
    {
        ...
        else
        {
			...
			err_code = DispatcherAddEvnt(DISP_TYPE_SINGLE, timeout_ticks, (CbPtr)update_timeout_handler, 0);
			if(err_code != 0) m_conn_params_timer_id = err_code;
			
            if((err_code == 0) && (m_conn_params_config.error_handler != NULL))
                m_conn_params_config.error_handler(NRF_ERROR_BUSY);
        }
    }
    else
    {
